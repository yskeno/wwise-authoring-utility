...
class MainWindow(tkinter.Tk):
    def __init__(self):
        ...
        self.iconbitmap("WwiseAuthoringUtility.ico")
    ...
    def open_custom_switchassign_setting(self, switch_info: dict):
        ...
        self.wd_switch_setting.protocol("WM_DELETE_WINDOW", self.close_window)

        self.frm_group_setting = ttk.Frame(self.wd_switch_setting, padding=5)
        self.lbl_groupname = ttk.Label(self.frm_group_setting, text="Select State/Switch Group to assign.")
        self.lbl_groupname.pack(anchor="nw")

        # Create GroupName ComboBox.
        cmb_values = [group_info.get("path", "") for group_info in self.switch_dict.values()]
        groupname_length = len(max(cmb_values)) + 15
        self.cmb_groupname = ttk.Combobox(
            self.frm_group_setting, width=groupname_length, state="readonly", values=cmb_values
        )
        # self.cmb_groupname.current(0)
        self.cmb_groupname.bind("<<ComboboxSelected>>", self._open_switch_keyword_setting)
        ...
        self.cvs_scrollable.create_window((0, 0), window=self.frm_scrollable, anchor="nw", tags="frame")
        self.frm_scrollable.columnconfigure(0, weight=1)
        self.frm_scrollable.columnconfigure(1, weight=2)
        self.frm_scrollable.bind(
            "<Configure>",
            lambda e: self.cvs_scrollable.configure(width=e.width, scrollregion=self.cvs_scrollable.bbox("all")),
        )
        self.cvs_scrollable.bind("<Configure>", self._on_canvas_configure)

        self.vsb = ttk.Scrollbar(self.wd_switch_setting, command=self.cvs_scrollable.yview)
        self.vsb.pack(side="right", fill="y")
        self.cvs_scrollable.configure(yscrollcommand=self.vsb.set)

        self.lbl_childlen = ttk.Label(
            self.wd_switch_setting, text="Enter keyword between State/Switch and audio object."
        )
        self.keyword_objects = {}

        self.frm_buttons = ttk.Frame(self.wd_switch_setting, padding=5)
        self.btn_assign = ttk.Button(
            self.frm_buttons, state="disable", text="Assign", command=self._assign_switch_keywords
        )
        self.btn_assign.pack(side="left", anchor="center")
        self.btn_cancel = ttk.Button(self.frm_buttons, state="normal", text="Cancel", command=self.close_window)
    ...
    def _on_canvas_configure(self, event):
        self.cvs_scrollable.itemconfigure("frame", width=self.cvs_scrollable.winfo_width())

    def _on_mouse_wheel_move(self, event=None):
        if event:
            self.cvs_scrollable.yview_scroll(int(-1 * (event.delta / abs(event.delta))), "units")
            ...
            if event.widget.get() == switch_info.get("path", ""):
                ...
                for child in switch_info.get("child", ""):
                    self.keyword_objects.setdefault(
                        ttk.Label(
                            self.frm_scrollable, name="lbl_" + child, width=childname_length, text=child, padding=2
                        ),
                        ttk.Entry(self.frm_scrollable, name="ent_" + child, width=childname_length),
                    )
    ...
    def _assign_switch_keywords(self):
        ...
        self.assigned_keywords = {
            keyword_entry.winfo_name().removeprefix("ent_"): keyword_entry.get()
            for keyword_entry in self.keyword_objects.values()
        }
    ...
    def show_progress_window(self):
        ...
        self.prog_bar = ttk.Progressbar(
            self, maximum=100, variable=self.__progress_rate, length=200, mode="determinate"
        )
