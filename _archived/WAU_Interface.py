...
class WwiseUtilityClient(WaapiClient):
    ...
    def _get_selected_objects_guid(self, type=""):
        selected = self.call("ak.wwise.ui.getSelectedObjects", {"options": {"return": ["id", "type"]}})["objects"]
        if type == "":
            return tuple(v for d in selected for k, v in d.items() if k == "id")
        else:
            return tuple(v for d in selected if d["type"] == type for k, v in d.items() if k == "id")
    ...
    def connect_to_localhost(self, window: MainWindow):
        ...
        window.after_idle(lambda: window.set_current_process(20, "Get Connection Status..."))
        connection_status: dict = self.call("ak.wwise.core.remote.getConnectionStatus")
        print(f"*** ConnectionStatus= {connection_status}")

        # --- Connect to Localhost ---
        if connection_status.get("isConnected", None) == False:
            window.after_idle(lambda: window.set_current_process(30, "Get Available Consoles..."))
            available_consoles: list = self.call("ak.wwise.core.remote.getAvailableConsoles")["consoles"]

            # Search Localhosts.
            window.after_idle(lambda: window.set_current_process(50, "Search localhosts..."))
            local_consoles = [console for console in available_consoles if console["host"] == "127.0.0.1"]
            ...
            window.after_idle(lambda: window.set_current_process(70, "Set target console..."))
            target_console: dict = {}
        ...
        # --- Disconnect from Localhost ---
            ...
            window.after_idle(lambda: window.set_current_process(50, "Disconnect from remote pratform..."))
            self.call("ak.wwise.core.remote.disconnect")
            window.after_idle(lambda: window.set_current_process(100, "Disconnected."))
    ...
    def auto_rename_container(self, window: MainWindow):
        ...
        for guid in guids:
            ...
            self.call("ak.wwise.core.object.setName", {"object": guid, "value": common_name})
        ...
        if len(failed_ids) > 0:
            if len(failed_ids) == len(guids):
                ...
                failed_names = "\n    ".join(self._get_name_from_guid(tuple(failed_ids)))
    ...
    def auto_assign_switch_container(self, window: MainWindow):
        ...
        for guid in guids:
            ...
            common_name: str = os.path.commonprefix(list(map(lambda object: object["name"], children)))
            if not common_name:
                failed_ids.add(guid)
                continue
            ...
            assignments: list = self.call("ak.wwise.core.switchContainer.getAssignments", {"id": guid})["return"]

            for child in children:
                target_name: str = re.sub("[_ -]*[0-9]+$", "", child["name"].replace(common_name, ""))
                for state in states:
                    if state["name"] == "None":
                        continue
    ...
    def get_stateandswitch_info(self):
        ...
        for group in group_list:
            ...
            for child in child_list:
                ret[group["id"]].setdefault("child", []).append(child["name"])
    ...
    def custom_assign_switch_container(self, window, stategrouptoassign: str, assigned_keywords: dict):
        ...
        for guid in guids:
            ...
            if (
                stategroup is None
                or stategroup["id"] == "{00000000-0000-0000-0000-000000000000}"
                or stategroup["name"] != os.path.basename(stategrouptoassign)
            ):
                failed_ids.add(guid)
                continue
                    ...
                    if keyword != "" and keyword in child["name"]:
                        for state in states:
                            if target_state in state["name"]:
                                for assignment in assignments:
                                    if (
                                        assignment["child"] == child["id"]
                                        and assignment["stateOrSwitch"] == state["id"]
                                    ):
                                        break
                                else:
                                    self.call(
        ...
        if len(failed_ids) > 0:
            if len(failed_ids) == len(guids):
                raise RuntimeError(f"Could not assign object(s):\n\n Check State or Switch keyword setting.")
            else:
                failed_names = "\n    ".join(self._get_name_from_guid(tuple(failed_ids)))
                raise RuntimeWarning(
                    f"Complete with Warning.\n\n Following SwitchContainer(s) not assigned:\n    {failed_names}"
